(() => {
  ...
  window.openDemoReview = (title = 'Review your demo', description = '') => {
    ...
    const selectionLine = [result?.innerText, selectedTime].filter(Boolean).join(' · ');
    dialog.querySelector('#demoReviewSummary').textContent = description || [choices.join(' · '), selectionLine || 'Explore the options, then return here to complete the simulated journey.'].join('\n');
